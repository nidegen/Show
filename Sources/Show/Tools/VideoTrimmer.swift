...
class VideoTrimmer {
  ...
  private static var trimError: Error {
    return NSError(domain: "com.github.nidegen.show", code: -1, userInfo: nil) as Error
  }
  ...
  func trimVideo(sourceURL: URL, destinationURL: URL, duration: Double, completion: @escaping (Result<URL, Error>) -> ()) {
    ...
    if CMTimeGetSeconds(asset.duration) < duration {
      completion(.success(sourceURL))
      return
    }
    
    let end = CMTime(seconds: duration, preferredTimescale: 1000)
    let begin = CMTime(seconds: 0, preferredTimescale: 1000)
    trimVideo(asset: asset, destinationURL: destinationURL, trimPoints: [(begin, end)], completion: completion)
  }
  ...
  func trimVideo(sourceURL: URL, destinationURL: URL,
    ...
    let asset = AVURLAsset(url: sourceURL, options: options)
    trimVideo(asset: asset, destinationURL: destinationURL, trimPoints: trimPoints, completion: completion)
  }
  
  func trimVideo(asset: AVURLAsset, destinationURL: URL,
                 trimPoints: TrimPoints,
                 preferredPreset: String = AVAssetExportPresetPassthrough,
                 completion: @escaping (Result<URL, Error>) -> Void) {
